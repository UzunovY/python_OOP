...
class PizzaDelivery:
    ...
    def make_order(self) -> Union[str, tuple[str, dict, str]]:
        self.ordered = True
        formatted_ingredients = [
            f"{i}: {q}" for i, q in self.ingredients.items()
        ]
        return (
            f"You've ordered pizza {self.name} prepared"
            f" with {', '.join(formatted_ingredients)} "
            f"and the price will be {self.price}lv."
        )

    def on_the_go(self) -> str:
